...
if (json_last_error() === JSON_ERROR_NONE) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        {
            $query = "select * from events GROUP BY tracking_number";
            $result = mysqli_query($con, $query);
            while ($row = mysqli_fetch_assoc($result)) {
                $track = $row['tracking_number'];
                $query1 = "SELECT COUNT(*) as total FROM events WHERE tracking_number='" . $track . "' ORDER BY date_stamp desc";
                $result1 = mysqli_query($con, $query1);
                $data = mysqli_fetch_assoc($result1);
                $query2 = "SELECT * FROM events WHERE tracking_number='" . $track . "' ORDER BY date_stamp desc";
