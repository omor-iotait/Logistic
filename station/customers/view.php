...
$total_pages = $con->query('SELECT * FROM customers WHERE created_by='.$station_id.' ')->num_rows;
// Check if the page number is specified and check if it's a number, if not return the default page number which is 1.
$page = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
// Number of results to show on each page.
$num_results_on_page = 1;

if ($stmt = $con->prepare('SELECT * FROM customers ORDER BY id LIMIT ?,?')) {
    $calc_page = ($page - 1) * $num_results_on_page;
    $query = 'SELECT * FROM customers WHERE created_by='.$station_id.' ORDER BY id LIMIT '.$calc_page.','.$num_results_on_page.'';
